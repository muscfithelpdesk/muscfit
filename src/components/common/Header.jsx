...
export default function Header({ topOffset = 0, isFixed = true }) {
  ...
  return (
    <>
      ...
      >
        <div
          className={`flex flex-col items-center justify-between transition-all duration-300 px-4 md:px-8 lg:px-12 ${scrolled ? 'py-2 md:h-[96px]' : 'py-2 md:h-[136px]'}`}
        >
          {/* Row 1: Top Bar (Mobile) / Main Bar (Desktop) */}
          <div className="w-full flex items-center justify-between h-[60px] md:h-full relative">

            {/* Left Group: Logo and Nav (Reverted to flex container for layout inside Row 1) */}
            ...
            {/* Right Group: Desktop Search and Icons (Mobile: Just Icons, moved out of Left Group logic if needed, but here we are reusing) */}
            {/* On mobile, we need the Right Group to be part of the flex row above. 
              The current structure has two main parent divs: Left Group (Logo+Nav) and Right Group.
              My change above wrapped "Left Group" content into a "Top Row" div for mobile. 
              Now I need to move the "Right Group" content INTO that same "Top Row" div for mobile, OR make the parent flex-col.
              
              Correction: I changed the main container to flex-col for mobile.
              So "Left Group" (renamed to Top Row container) now needs to contain everything for the top row.
          */}

            {/* Right Group: Search and Icons */}
            <div className="flex items-center justify-end h-full gap-2 md:gap-3 overflow-visible flex-shrink-0 md:flex-1">
              {!isAdminPage && (
              ...
              <div className="flex items-center gap-1.5 md:gap-2.5 flex-shrink-0">
                ...
                {/* Mobile Search Icon - HIDDEN (We have full bar now) */}
                <div className="relative group hidden items-center" ref={searchRef}>
                  {/* hidden class added above to completely hide it on all breakpoints since we have desktop search and mobile full bar */}
                  <button
          ...
          <div className="w-full md:hidden pb-4 px-1">
            ...
            >
              ...
              <div className="block w-full h-10 pl-11 pr-4 bg-muted/30 border border-border/50 rounded-full text-sm font-medium flex items-center text-text-secondary select-none">
