        ...
        <div className="flex items-center justify-center min-h-[80vh]">
          <div className="text-center">
            ...
            <button
              ...
              className="px-6 py-3 bg-foreground text-background font-semibold rounded-lg hover:bg-black/80 transition"
            >
              Back to Orders
            </button>
          </div>
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <button
            onClick={() => router?.push('/user-profile')}
            className="text-foreground hover:text-text-secondary mb-4 flex items-center gap-2"
          >
        ...
        <div className="bg-surface rounded-lg p-6 mb-8 border border-border">
          <h2 className="text-xl font-semibold mb-6 text-foreground">Delivery Status</h2>
              ...
              {['order_confirmed', 'processing', 'shipped', 'out_for_delivery', 'delivered']?.map((status, index) => {
                ...
                return (
                  <div key={status} className="flex flex-col items-center">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center mb-2 transition-all ${isCompleted ? 'bg-black text-white' : 'bg-muted text-text-secondary'
                    ...
                    <p className={`text-xs text-center max-w-[80px] ${isCompleted ? 'text-black font-semibold' : 'text-text-secondary'}`}>
            ...
            <div className="mt-8 p-4 bg-muted/30 rounded-lg">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-lg font-semibold text-foreground">{formatDate(latestTracking?.estimatedDelivery)}</p>
                </div>
                {latestTracking?.courierName && (
                  <div className="text-right">
                    <p className="text-sm text-text-secondary mb-1">Courier</p>
                    <p className="font-semibold text-foreground">{latestTracking?.courierName}</p>
                  </div>
                )}
              </div>
              {latestTracking?.trackingNumber && (
                <div className="mt-4 pt-4 border-t border-border">
                  ...
                  <div className="flex items-center gap-2">
                    <p className="font-mono font-semibold text-foreground">{latestTracking?.trackingNumber}</p>
                    <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
                ...
                {order?.orderItems?.map((item) => (
                  <div key={item?.id} className="flex gap-4 pb-4 border-b border-border last:border-0">
                    ...
                    <div className="flex-1">
                      <h3 className="font-semibold mb-1 text-foreground">{item?.productName}</h3>
                      {item?.product?.brand && (
                        <p className="text-sm text-text-secondary mb-1">{item?.product?.brand}</p>
                      )}
                    ...
                    <div className="text-right">
                      ...
                      {item?.discountAmount > 0 && (
                        <p className="text-sm text-text-secondary line-through">{formatCurrency(item?.unitPrice * item?.quantity)}</p>
                      )}
                    </div>
                  </div>
                ))}
            ...
            <div className="bg-surface rounded-lg p-6 border border-border">
              <h2 className="text-xl font-semibold mb-4 text-foreground">Tracking History</h2>
              <div className="space-y-4">
                  ...
                  <div key={track?.id} className="flex gap-4">
                    ...
                    <div className="flex-1 pb-6">
                      <div className="flex items-start justify-between mb-1">
                        <h3 className="font-semibold text-foreground">{getStatusLabel(track?.status)}</h3>
                        <span className="text-sm text-text-secondary">{formatDate(track?.createdAt)}</span>
                      </div>
                      <p className="text-text-secondary text-sm mb-1">{track?.description}</p>
                      {track?.location && (
          ...
          {/* Order Summary */}
          <div>
            <div className="bg-surface rounded-lg p-6 mb-8 sticky top-4 border border-border">
              ...
              <div className="space-y-3 mb-4 pb-4 border-b border-border">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-foreground">{formatCurrency(order?.taxAmount)}</span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="text-sm space-y-1">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium capitalize text-foreground">{order?.paymentMethod?.replace('_', ' ')}</span>
                  </div>
                  <div className="flex justify-between">
              ...
              <div className="mt-6 p-4 bg-muted/50 rounded-lg">
                <h3 className="font-semibold mb-2 text-sm text-foreground">Need Help?</h3>
                <p className="text-xs text-text-secondary mb-3">Contact our customer support for any queries</p>
                <button className="w-full px-4 py-2 bg-foreground text-background rounded-lg hover:bg-black/80 transition text-sm font-medium">
                  Contact Support
                </button>
              </div>
            </div>
