  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing && currentProduct) {
        ...
        console.log('🔄 Pushing update to Supabase:', currentProduct.id);

        // Push to Supabase first and wait for response
        await productService.updateProduct(currentProduct.id, updateData);
        console.log('✅ Supabase updated successfully');

        resetForm();
        await fetchProducts(); // Refresh website data after DB update
        alert('Product updated successfully in Supabase!');
      } else {
        console.log('🆕 Creating new product in Supabase...');
        ...
        resetForm();
        await fetchProducts(); // Refresh website data after DB creation
        alert('Product created successfully in Supabase!');
      }
    } catch (err) {
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-4 pt-4">
                  <button
                    type="submit"
                    disabled={isSaving}
                    className="flex-1 bg-black text-white font-bold py-3 rounded hover:bg-zinc-800 transition disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                ...
                <table className="w-full text-left">
                  <thead className="bg-gray-100 border-b border-gray-200">
                    <tr>
                      ...
                      <th className="p-4 font-bold text-xs uppercase text-gray-500">
                        Original Price
                      </th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {products.map((product) => (
                      ...
                      >
                        ...
                        <td className="p-4 text-sm">{product.stockQuantity || 0}</td>
                        <td className="p-4">
                          <span
                            className={`px-2 py-1 text-xs rounded-full font-medium ${product.isActive
